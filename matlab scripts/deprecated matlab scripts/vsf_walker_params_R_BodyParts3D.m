...
% foot_y = 7;
% foot_z = 7;
foot_x = subj1.seg.footX;
foot_y = subj1.seg.footY;
foot_z = subj1.seg.footZ;
...
lower_leg_length = subj1.seg.shankLengthL;
upper_leg_length = subj1.seg.legLengthL;
lower_leg_length_amputated = lower_leg_length*res_shank_length;
pylon_radius = 1.5; %cm
socket2shank_offset = [1.5 1 25.5]; %cm 
pylon_length = lower_leg_length - lower_leg_length_amputated - socket2shank_offset(3) + 15.5; %cm
pylon_density = 50000; %kg/m^3

%Torso parameters (cm)
torso_y = subj1.seg.pelvisY;
torso_x = subj1.seg.pelvisX;
torso_z = subj1.seg.pelvisZ;
torso_offset_z = -.1;
torso_offset_x = -0.5;
torso_offset_y = .1;
...
init_height = foot_z + subj1.seg.legLengthL + subj1.seg.shankLengthL + ...
    subj1.seg.pelvisZ/2 + torso_offset_z+ height_plane/2;
...
if isequal(ModelControl,0)
    ...
    pelvisPosY = [time,-1*subj1.km.PelvisCOMPos(:,2)];  %Not sure about this one, might be positive
    pelvisPosZ = [time,subj1.km.PelvisCOMPos(:,3)];
    pelvisAngX = [time,deg2rad(subj1.km.PelvisAngle(:,1))];
    pelvisAngY = [time,-1*deg2rad(subj1.km.PelvisAngle(:,2))];
...
init_height = foot_z + lower_leg_length + upper_leg_length + ...
              torso_z/2 + torso_offset_z + height_plane/2 + 5;
