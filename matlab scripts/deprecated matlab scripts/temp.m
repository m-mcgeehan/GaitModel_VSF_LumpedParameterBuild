...
shankRAng_legR = wrapTo180(SpinCalc('DCMtoEA123',((inv(legRR))*shankRR),.3,1)); %precision tolerance violation 
shankLAng_legL = wrapTo180(SpinCalc('DCMtoEA123',((inv(legLR))*shankLR),.2,1));
footRAng_shankR = wrapTo180(SpinCalc('DCMtoEA123',((inv(shankRR))*footRR),.35,1)); %precision tolerance violation 
