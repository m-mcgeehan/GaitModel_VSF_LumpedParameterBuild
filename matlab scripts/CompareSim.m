%% Compare forces
VSFGRFR_exp = downsample(sqrt(subj1.forces.FP2(200:475,3).^2 + subj1.forces.FP2(200:475,2).^2 + ...
    subj1.forces.FP2(200:475,1).^2),2); 
VSFtime_exp = downsample(time(200:475),2);
VSFGRFR_exp = timeseries(VSFGRFR_exp,VSFtime_exp);

intactGRFR_exp = downsample(sqrt(subj1.forces.FP1(160:350,3).^2 + subj1.forces.FP1(160:350,2).^2 ...
    + subj1.forces.FP1(160:350,1).^2),2);
intactTime_exp = downsample(time(160:350),2);
intactGRFR_exp = timeseries(intactGRFR_exp,intactTime_exp);

VSFGRFR_sim = SDOSimTest_Log.VSF_CF;
...
figure;
subplot(1,2,1); plot(VSFGRFR_exp.time, VSFGRFR_interp,'k'); hold on; plot(VSFGRFR_exp,'r');...
    legend('simulation','experimental'); title(['R^2 = ', num2str(VSFGRFR_r2), ' RMSE =', num2str(VSF_RMSE), 'N']);
subplot(1,2,2); plot(intactGRFR_exp.time, intactGRFR_interp,'k'); hold on; plot(intactGRFR_exp,'r'); legend('simulation','experimental');
    legend('simulation','experimental'); title(['R^2 = ', num2str(intactGRFR_r2), ' RMSE =', num2str(intact_RMSE), 'N']);
sgtitle(strcat('Resultant GRF for: ', subj1.trial));
...
figure;
subplot(3,2,1); plot(LHipSim(1:idx),'k'); hold on; plot(rad2deg(hipLAngY(1:idx,2)),'r');
    title('LHip');legend('simulation','experimental'); title(['R^2 = ', num2str(LHip_r2), ' RMSE = ', num2str(LHip_RMSE), 'deg']);
subplot(3,2,2); plot(RHipSim(1:idx),'k'); hold on; plot(rad2deg(hipRAngY(1:idx,2)),'r');
    title('RHip');legend('simulation','experimental'); title(['R^2 = ', num2str(RHip_r2), ' RMSE = ', num2str(RHip_RMSE), 'deg']);
subplot(3,2,3); plot(LKneeSim(1:idx),'k'); hold on; plot(rad2deg(kneeLAngY(1:idx,2)),'r');
    title('LKnee');legend('simulation','experimental'); title(['R^2 = ', num2str(LKnee_r2), ' RMSE = ', num2str(LKnee_RMSE), 'deg']);
subplot(3,2,4); plot(RKneeSim(1:idx),'k'); hold on; plot(rad2deg(kneeRAngY(1:idx,2)),'r');
    title('RKnee');legend('simulation','experimental'); title(['R^2 = ', num2str(RKnee_r2), ' RMSE = ', num2str(RKnee_RMSE), 'deg']);
subplot(3,2,5); plot(RAnkleSim(1:idx),'k'); hold on; plot(rad2deg(ankle_motionRY(1:idx,2)),'r');
    title('RAnkle');legend('simulation','experimental'); title(['R^2 = ', num2str(RAnkle_r2), ' RMSE = ', num2str(RAnkle_RMSE), 'deg']);
subplot(3,2,6); plot(LAnkleSim(1:idx),'k'); hold on; plot(rad2deg(ankleLAngY(1:idx,2)),'r');
    title('RAnkle');legend('simulation','experimental'); title(['R^2 = ', num2str(LAnkle_r2), ' RMSE = ', num2str(LAnkle_RMSE), 'deg']);
sgtitle(strcat('Simulation vs Experimental Pelvis kinematics for: ', subj1.trial));
...
% % 
% 
% time1 = linspace(0,gaitPeriod,length(subj1.km.Rhip(:,3)));
% figure;  
...
% 
% %% 
% a = linspace(0,gaitPeriod,length(subj1.forces.FP1));
% 
% FP1TS = timeseries(subj1.forces.FP1,a);
% FP2TS = timeseries(subj1.forces.FP2,a);
% 
% order = 2;
...
% plot(time1,subj1.forces.FP1(:,3)); hold on
% plot(time1,subj1.forces.FP2(:,3)); hold on
% xlim([.85 2.25]);

% 
% figure;  
% subplot(3,2,1);plot(timeSim,rad2deg(measR.hip_angle.data),'r');hold on;plot(time,subj1.km.Rhip(:,2),'k');
% title('Right Hip angle')
% subplot(3,2,2);plot(timeSim,rad2deg(measL.hip_angle.data),'r');hold on;plot(time,subj1.km.Lhip(:,2),'k');
% title('Left Hip angle')
% subplot(3,2,3);plot(timeSim,rad2deg(measR.knee_angle.data),'r');hold on;plot(time,subj1.km.Rknee(:,2),'k');
% title('Right Knee angle')
% subplot(3,2,4);plot(timeSim,rad2deg(measL.knee_angle.data),'r');hold on;plot(time,subj1.km.Lknee(:,2),'k');
% title('Left Knee angle')
% subplot(3,2,5);plot(timeSim,rad2deg(measR.ankle_angle.data),'r');hold on;plot(time,subj1.km.Rankle(:,2),'k');
% title('Right Ankle angle')
% subplot(3,2,6);plot(timeSim,rad2deg(measL.ankle_angle.data),'r');hold on;plot(time,subj1.km.Lankle(:,2),'k');
% title('Left Ankle angle')
% sgtitle('Simulation vs Experimental kinematics (red = sim, black = exp)');