...
markerNames = {'RTME' 'RTLA' 'RTH4' 'RTH3' 'RTH2' 'RTH1' 'RTCE' 'RPSI' 'RMEP' 'RLEP' 'RHME' 'RHLA' 'RHCE' 'RGTR' 'RASI' 'LTME'...
    'LTLA' 'LTH4' 'LTH3' 'LTH2' 'LTCE' 'LSK4' 'LSK3' 'LSK2' 'LSK1' 'LPSI' 'LMEP' 'LLEP' 'LHME' 'LHLA' 'LHCE' 'LGTR' 'LASI'...
    'RKJC' 'RHJC_reg' 'RHJC_fxnl' 'RAJC_fxnl' 'LKJC' 'LHJC_reg' 'LHJC_fxnl' 'LAJC_fxnl'};
...
%     shankR_k = (markerSet.KJCR(i,:) - markerSet.AJC_pros(i,:))/(norm(markerSet.KJCR(i,:) - markerSet.AJC_pros(i,:)));
    shankR_k = (markerSet.KJCR(i,:) - markerSet.RAJC_fxnl(i,:))/(norm(markerSet.KJCR(i,:) - markerSet.RAJC_fxnl(i,:)));
...
%    shankL_k = (markerSet.KJCL(i,:) - markerSet.AJCL(i,:))/(norm(markerSet.KJCL(i,:) - markerSet.AJCL(i,:)));
   shankL_k = (markerSet.KJCL(i,:) - markerSet.LAJC_fxnl(i,:))/(norm(markerSet.KJCL(i,:) - markerSet.LAJC_fxnl(i,:)));
...
dynamic.km.PelvisCOMPos = pelvisOrigin;
dynamic.markers = markerSet;
dynamic.markersTS = markerSetTS;
dynamic.forces.FP1 = FP1ds;
dynamic.forces.FP2 = FP2ds;
dynamic.framerate = FRAME_RATE;
dynamic.static = static;
dynamic.trial = extractAfter(string(FILE_NAME{1}),"Visual3D\"); 

clearvars -except dynamic static

%% name and save data
trialInfo = replace(dynamic.trial,'-','_');
varName = strcat(extractBefore(static.trial,"\"),"_",(extractBetween(trialInfo,'c3d\','.')));

eval([(convertStringsToChars(varName)) '=dynamic']);

uisave((convertStringsToChars(varName)),(convertStringsToChars(varName)));
clear trialInfo varName dyanmic